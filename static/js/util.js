...
Util.getIndexInArray = function(array, el) {
	if (el.classList.contains('cd-h-timeline__date') != true) {
		console.log('ERROR!')
		var element = el.closest('.cd-h-timeline__date');
	} else {
		var element = el;
	}
	let result = Array.prototype.indexOf.call(array, element);
	return result
};
...
$(document).ready(function() {
	...
	for (let i = 0; i < timelineDots.length; i++) {
		...
		if (position == 0) {
			...
			let newDomR3 = '<div class="timeline_rivets" style="opacity: 0.75; width: 25px; height: 20px; right: 74px;"></div>' + '<div class="timeline_rivets" style="opacity: 0.75; top:20px; width: 25px; height: 10px; right: 73.5px; border-left: 25px solid transparent; border-bottom: 10px solid black;"></div>'
			let shell = '<div style="pointer-events:none; bottom:8px; width: 225px; height: 40px;position: absolute; transform: scaleY(-1)">' + newDomL1 + newDomL2 + newDomL3 + newDomCenter + newDomR1 + newDomR2 + newDomR3 + '</div>';
			$(shell).appendTo(dot);
		...
		if (position == 2) {
			...
			let shell = '<div style="pointer-events:none; top:15px; width: 225px; height: 40px;position: absolute;">' + newDomL1 + newDomL2 + newDomL3 + newDomCenter + newDomR1 + newDomR2 + newDomR3 + '</div>';
			$(shell).appendTo(dot);
			// $(bracket).appendTo(dot);  
		}
	}
