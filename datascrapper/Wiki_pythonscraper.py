...
def parseFirstPage(db):
    ...
    for tr in soup.find_all('tr')[2:]:
        ...
        # print ('Name: ', name)
        # Insert into DB
        db.execute("INSERT INTO 'Movies' ('id','Name', 'Year', 'YearText', 'Future', 'FutureText', 'Setting','Plot') VALUES (NULL, :name, :year, :yeartext, :future, :futuretext, :setting, NULL)", name = str(name), year=year,yeartext = str(yearText), future=future, futuretext = str(futureText), setting= str(setting))
...
def encoder(toencode):
    ...
    if isinstance(toencode, str):
        toencode = toencode.decode('ascii', 'ignore').encode('ascii') #note: this removes the character and encodes back to string.
